...
+ (NSString *)descriptionWithEvent:(AVPEventType)event{
    ...
    switch (event) {
        case AVPEventPrepareDone:
            eventString = NSLocalizedString(@"准备成功" , nil);
            break;
        case AVPEventAutoPlayStart:
            eventString = NSLocalizedString(@"开始播放" , nil);
            break;
        case AVPEventFirstRenderedStart:
            eventString = NSLocalizedString(@"第一帧加载完成", nil);
            break;
        case AVPEventCompletion:
            eventString = NSLocalizedString(@"播放已结束", nil);
            break;
        case AVPEventLoadingStart:
            eventString = NSLocalizedString(@"开始加载视频数据", nil);
            break;
        case AVPEventLoadingEnd:
            eventString = NSLocalizedString(@"视频资源加载完成", nil);
            break;
        case AVPEventSeekEnd:
            eventString = NSLocalizedString(@"跳转成功" , nil);
            break;
        case AVPEventLoopingStart:
            eventString = NSLocalizedString(@"循环播放开始事件" , nil);
            break;            
        default:
            break;
    }
