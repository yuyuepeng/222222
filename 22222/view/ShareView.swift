...
//  JZSPin
//
//  Created by 崔宝帅 on 2019/9/18.
//  Copyright © 2019 Sunday. All rights reserved.
//
import UIKit
class ShareView: UIView {
    
    var safeAreaBottomHeight: CGFloat = isiPhoneX ? 30 : 0
    ...
    func createUI() {
        ...
        container.frame = CGRect.init(x: 0, y: kScreenHeight, width: kScreenWidth, height: 280 + safeAreaBottomHeight)
        container.isUserInteractionEnabled = true
        container.backgroundColor = UIColor.randomColor()
        ...
        let label = UILabel.init(frame: CGRect.init(origin: .zero, size: CGSize.init(width: kScreenWidth, height: 35)))
        label.textAlignment = .center
        label.numberOfLines = 0
        label.text = "分享到"
        label.textColor = UIColor.randomColor()
        label.font = FontOfSize(x: 15)
        container.addSubview(label)
        
        let itemWidth = 68
        let topScrollView = UIScrollView.init(frame: CGRect.init(x: 0, y: 35, width: kScreenWidth, height: 90))
        ...
        let bottomScrollView = UIScrollView.init(frame: CGRect.init(x: 0, y: 135, width: kScreenWidth, height: 90))
        ...
        cancel.frame = CGRect.init(x: 0, y: 230, width: kScreenWidth, height: 50 + safeAreaBottomHeight)
        cancel.titleEdgeInsets = UIEdgeInsets(top: -safeAreaBottomHeight, left: 0, bottom: 0, right: 0)
        cancel.setTitle("取消", for: .normal)
        cancel.setTitleColor(UIColor.randomColor(), for: .normal)
        cancel.titleLabel?.font = FontOfSize(x: 17)
        cancel.backgroundColor = UIColor.randomColor()
    ...
    @objc func onShareItemTap(sender:UITapGestureRecognizer) {
        ...
        HLTMBProgressHUB.ShowMessage(text: "nbnbnbnbnb")
        dismiss()
    }
